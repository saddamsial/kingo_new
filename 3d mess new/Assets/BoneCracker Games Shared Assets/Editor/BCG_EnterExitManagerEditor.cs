...
public class BCG_EnterExitManagerEditor : Editor {
	...
	public static void CreateEnterExitManager(){
		...
		if(GameObject.FindObjectOfType<BCG_EnterExitManager>()){
			...
			EditorUtility.DisplayDialog("Scene has _BCGEnterExitManager already!", "Scene has _BCGEnterExitManager already!", "Close");
	...
	public static void CreateEnterExitVehicle(){
		...
		if (Selection.activeGameObject == null) {
			EditorUtility.DisplayDialog ("Select your vehicle on your scene, and then come back again!", "Select your vehicle on your scene, and then come back again.", "Close");
			return;
		}

		if (Selection.activeGameObject.GetComponentInParent<RCC_CarControllerV3>() == null) {
			EditorUtility.DisplayDialog ("Selected vehicle doesn't have RCC_CarControllerV3!", "Selected vehicle doesn't have RCC_CarControllerV3! You must have a running vehicle before the Enter-Exit System.", "Close");
			return;
		}

		if(Selection.activeGameObject.GetComponentInParent<BCG_EnterExitVehicle>()){

			EditorUtility.DisplayDialog("Selected vehicle has BCG_EnterExitVehicle already!", "Selected vehicle has BCG_EnterExitVehicle already!", "Close");
	...
	public static void CreateEnterExitPlayer(){
		...
		if (Selection.activeGameObject == null) {
			EditorUtility.DisplayDialog ("Select your FPS player on your scene, and then come back again!", "Select your FPS player on your scene, and then come back again.", "Close");
			return;
		}

		if(Selection.activeGameObject.GetComponentInParent<BCG_EnterExitPlayer>()){

			EditorUtility.DisplayDialog("Selected FPS Player has BCG_EnterExitPlayer already!", "Selected FPS Player has BCG_EnterExitPlayer already!", "Close");
