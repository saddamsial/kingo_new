...
namespace Invector.vShooter
{
    ...
    public class vShooterManager : vMonoBehaviour
    {
        ...
        private int secundaryExtraAmmo;

        public virtual int ExtraAmmo => extraAmmo;
        public virtual int SecundaryExtraAmmo => secundaryExtraAmmo;
        ...
        public virtual int GetShotID(bool secundaryWeapon= false)
        {
            ...
            if (rWeapon && rWeapon.gameObject.activeInHierarchy) id = (secundaryWeapon && rWeapon.secundaryWeapon)?(int)rWeapon.secundaryWeapon.shotID: (int)rWeapon.shotID;
            else if (lWeapon && lWeapon.gameObject.activeInHierarchy) id = (secundaryWeapon && lWeapon.secundaryWeapon) ? (int)lWeapon.secundaryWeapon.shotID : (int)lWeapon.shotID;
            return id;
        }
        ...
        {
            var hasAmmo = secundaryWeapon ? secundaryExtraAmmo > 0 : extraAmmo > 0;
            return hasAmmo;
        }
        ...
        public virtual void UpdateLeftAmmo()
        {
            ...
            UpdateTotalAmmo(lWeapon.secundaryWeapon, ref secundaryExtraAmmo, -1);
        }
        ...
        public virtual void UpdateRightAmmo()
        {
            ...
            UpdateTotalAmmo(rWeapon.secundaryWeapon, ref secundaryExtraAmmo, 1);
        }
        ...
        protected virtual void UpdateAmmoDisplay(int displayId)
        {
            ...
            if (useAmmoDisplay && ammoDisplay)
            {
                ...
                {
                    string display1 = weapon.dontUseReload ? weapon.isInfinityAmmo ? "Infinity" : (weapon.ammoCount + extraAmmo).ToString() : (string.Format("{0} | {1}", weapon.ammoCount, (weapon.isInfinityAmmo ? "Infinity" : extraAmmo.ToString())));
                    string display2 = weapon.secundaryWeapon.dontUseReload ? weapon.secundaryWeapon.isInfinityAmmo ? "Infinity" : (weapon.secundaryWeapon.ammoCount + secundaryExtraAmmo).ToString() : (string.Format("{0} | {1}", weapon.secundaryWeapon.ammoCount, (weapon.secundaryWeapon.isInfinityAmmo ? "Infinity" : secundaryExtraAmmo.ToString())));
                    ammoDisplay.UpdateDisplay(display1,display2, weapon.ammoID);
                }
                else
                {
                    string text = weapon.dontUseReload ? weapon.isInfinityAmmo ? "Infinity" : (weapon.ammoCount + extraAmmo).ToString() : (string.Format("{0} | {1}", weapon.ammoCount, (weapon.isInfinityAmmo ? "Infinity" : extraAmmo.ToString())));
        ...
        public virtual void Shoot(Vector3 aimPosition, bool applyHipfirePrecision = false, bool useSecundaryWeapon = false)
        {
            ...
            else if (targetWeapon.autoReload && targetWeapon.ammoCount <= 0 && WeaponHasUnloadedAmmo(useSecundaryWeapon))
            {
                ReloadWeapon();
                return;
            }
            ...
            if (!useSecundaryWeapon && extraAmmo <= 0)
            {
                weapon.onFinishAmmo.Invoke();
            }
            else if (useSecundaryWeapon && secundaryExtraAmmo <= 0 && weapon.secundaryWeapon) weapon.secundaryWeapon.onFinishAmmo.Invoke();
        }
