...
namespace MTAssets.EasyMeshCombiner.Editor
{
    public class MeshCombinerTool : EditorWindow
    {
        ...
        */
        ...
        private Transform targetParentOfCombinedMeshInHierarchy = null;
        ...
        void OnDestroy()
        {
            ...
            //Clear selected gameobjects to show
        ...
        void OnGUI()
        {
            ...
            scrollPosPreferences = EditorGUILayout.BeginScrollView(scrollPosPreferences, GUILayout.Width(284), GUILayout.Height(365));
            EditorGUILayout.LabelField("Preferences of Merge", tituloBox);

            GUILayout.Space(20);
            int spaceToRememberMessage = 156;
        ...
        void ValidateSelection()
        {
            ...
            if (gameObjectsSelected.Length > 0)
            {
                targetParentOfCombinedMeshInHierarchy = gameObjectsSelected[0].transform.parent;
                targetIndexOfCombinedMeshInHierarchy = gameObjectsSelected[0].transform.GetSiblingIndex() + 1;
            ...
            for (int i = 0; i < foundGameObjects.Count; i++)
            {
                ...
                if (mf != null || mr != null)
                {
                    ...
                    if (meshCombinerPreferences.combineInactives == false && foundGameObjects[i].gameObject.activeSelf == false)
                        continue;
                    if (meshCombinerPreferences.combineInactives == false && foundGameObjects[i].gameObject.activeInHierarchy == false)
                        continue;
        ...
        void CombineMeshes()
        {
            //Prepare the sufix title, if support to lightmap is enabled
            string suffixDescription = "";
            if (meshCombinerPreferences.lightmapSupport == true)
                suffixDescription = ". Processing lightmaps";

            //Show progress dialog
            EditorUtility.DisplayProgressBar("Merging", "A moment" + suffixDescription + "...", 1);
            ...
            holderGameObject.transform.SetParent(targetParentOfCombinedMeshInHierarchy);
